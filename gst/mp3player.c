...
int main(int argc,char **argv)
{
	...
	if(!(decoder = gst_element_factory_make("mad","mad-decoder")))  
	{
	...
	gst_bin_add_many(GST_BIN(pipeline),source,decoder,sink,NULL);//将所有的元素都添加到箱柜中 
//  gst_bin_add_many(GstBin *bin, GstElement * elementl ,...)  elementl : 元素列表 ... ：附加元素不是GstElement

	gst_element_link_many(source,decoder,sink,NULL);//unlink 解除link
